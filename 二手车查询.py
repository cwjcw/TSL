...
def get_response():
    try:
        response = requests.get(CAR_URL, headers=headers_computer, proxies=PROXIES, verify=False, timeout=5)  # 使用指定请求头、代理
        return response
    except Exception as e:
        print("请求异常，稍后重试，异常信息:" + str(e))
...
def query_car():
    ...
    try:
        ...
        response_json = codecs.decode(response_text, 'unicode_escape')
...
def analyze(response_dic):
    ...
    for index, result in enumerate(response_dic['results'], 1):
        ...
        型号 = result['TrimName'][:result['TrimName'].find("版")+1]
        车架号 = result['VIN']
        ...
        if 车架号 in last_vin_info_dic.keys():  # 如果车架号在上次查询的结果中
            if last_vin_info_dic[车架号]['售价'] != 售价:  # 如果价格有变化
                my_logger.info(f"价格变化：之前售价{last_vin_info_dic[车架号]['售价']}，当前售价{售价}，降价{str(int(last_vin_info_dic[车架号]['售价']) - int(售价))}，详细信息：{info_str}")
                notify_pricechange.append(f"之前售价{last_vin_info_dic[车架号]['售价']}，当前售价{售价}，降价{str(int(last_vin_info_dic[车架号]['售价']) - int(售价))}，详细信息：{info_str}")
...
if __name__ == '__main__':
    while True:
        query_car()
        time.sleep(300 + random.randint(0, 20))